...
@section('content')
    ...
    <div class="p-3">
        <div class="card shadow col-lg-6">
            ...
            <form id="blogForm" class="p-3 fs" action="" method="POST" enctype="multipart/form-data">
                @csrf
                @if ($blog->thumbnail)
                <div class="mb-3">
                    <img width="100" src="{{ asset("uploads/$blog->thumbnail") }}" alt="">
                </div>
                @endif
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control name" value="{{ old('title', $blog->title) }}" id="title" placeholder="Enter blog title">
                    @error('title')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" name="body" id="body" cols="30" rows="5" placeholder="Enter blog body">{{ old('body', $blog->body) }}</textarea>
                    @error('body')
                        <div class="text-danger mt-2">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <button type="submit" class="create-btn btn btn-primary">Update</button>
            </form>
        </div>
    </div>
        ...
        $(document).ready(function () {
            ...
            $('#blogForm').on('submit', function(e){
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: "{{ route('updateBlog', $blog->token) }}",
                    statusCode: {
                        404: function() {
                            alert('404')
                        },
                    ...
                    success: function (response) {
                        if (response.status == true){
                            window.location.href = "{{ route('viewBlog', $blog->token) }}";
